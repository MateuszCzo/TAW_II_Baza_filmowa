...
const registerUser = async (request, response) => {
    ...
    try {
        if (password < 6) {
            return response.status(400).json({ error: "Password must be at least 6 characters logn." });
        }
        const specialChars = /[!@#$%^&*(),.?":{}|<>]/;
        if (!specialChars.test(password)) {
            return response.status(400).json({
                error: "Password must contain at least one special character." + password,
            });
        }
        ...
        await user.save();
        const token = generateJWT(user._id, user.name, user.role);
